"""
Cathie Wood innovation-focused investing analysis implementation.

Analyzes stocks using Wood's principles focused on disruptive innovation,
exponential growth potential, and long-term value creation.
"""
...
from stock_analyser.tools.tool_utils.analysis_constants import (
    ...
    RD_INTENSITY_HIGH,
    RD_INTENSITY_MODERATE,
    RD_INTENSITY_LOW,
    FCF_TO_NI_EXCELLENT,
    FCF_TO_NI_GOOD,
    FCF_TO_NI_MODERATE,
    OPERATING_MARGIN_STRONG,
    OPERATING_MARGIN_ACCEPTABLE,
    CAPEX_INTENSITY_MODERATE,
    WoodDCFDefaults,
    MARGIN_OF_SAFETY_GOOD,
    MARGIN_OF_SAFETY_ACCEPTABLE,
...
def calculate_cathie_wood_analysis_data(
    ticker: str,
    growth_rate: float = WoodDCFDefaults.GROWTH_RATE,
    discount_rate: float = WoodDCFDefaults.DISCOUNT_RATE,
    terminal_multiple: float = WoodDCFDefaults.TERMINAL_MULTIPLE,
    projection_years: int = WoodDCFDefaults.PROJECTION_YEARS,
) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        growth_rate: Annual growth rate for projections (default 20%)
        discount_rate: Discount rate for present value calculations (default 15%)
        terminal_multiple: Multiple for terminal value calculation (default 25x)
        projection_years: Number of years to project cash flows (default 5)

    Returns:
        Complete analysis with signal, score, and detailed components
    """
    try:
        disruptive_analysis = analyse_disruptive_potential(ticker)
    except Exception as e:
        disruptive_analysis = create_error_result(
            f"Error in disruptive potential analysis: {str(e)}"
        )

    try:
        innovation_analysis = analyse_innovation_growth(ticker)
    except Exception as e:
        innovation_analysis = create_error_result(
            f"Error in innovation growth analysis: {str(e)}"
        )
...
def analyse_disruptive_potential(ticker: str) -> Dict[str, Any]:
    """
    ...
    if revenue is not None and not revenue.empty:
        ...
        if len(revenue_values) >= 3:
            ...
            if len(growth_rates) >= 2:
                ...
                if latest_growth > REVENUE_GROWTH_EXCEPTIONAL:
                    score += 3
                    details.append(
                        f"Exceptional revenue growth: {(latest_growth * 100):.1f}%"
                    )
                elif latest_growth > REVENUE_GROWTH_STRONG:
                    score += 2
                    details.append(
                        f"Strong revenue growth: {(latest_growth * 100):.1f}%"
                    )
                elif latest_growth > REVENUE_GROWTH_MODERATE:
                    score += 1
                    details.append(
    ...
    if (
        ...
        gross_profit_values = filter_valid_values(gross_profit)
        total_revenue_values = filter_valid_values(total_revenue)

        if len(gross_profit_values) == len(total_revenue_values) and len(gross_profit_values) > 0:
            gross_margins = [
                safe_divide(gp, tr)
                for gp, tr in zip(gross_profit_values, total_revenue_values)
            ]
            ...
            if len(gross_margin_values) >= 2:
                ...
                if gross_margin_values[0] > GROSS_MARGIN_EXCELLENT:
                    score += 2
    ...
    if len(revenue_values) >= 2 and len(operating_expense_values) >= 2:
        rev_growth = safe_divide(
            revenue_values[0] - revenue_values[-1],
            abs(revenue_values[-1])
        )
        opex_growth = safe_divide(
    ...
    r_and_d_values = filter_valid_values(research_and_development) if research_and_development is not None else []

    if r_and_d_values and revenue_values:
        rd_intensity = safe_divide(r_and_d_values[0], revenue_values[0])
        if rd_intensity > RD_INTENSITY_HIGH:
            score += 3
            details.append(
                f"High R&D investment: {(rd_intensity * 100):.1f}% of revenue"
            )
        elif rd_intensity > RD_INTENSITY_MODERATE:
            score += 2
            details.append(
                f"Moderate R&D investment: {(rd_intensity * 100):.1f}% of revenue"
            )
        elif rd_intensity > RD_INTENSITY_LOW:
            score += 1
            details.append(
    ...
    normalized_score = round(safe_divide(score, max_score, 0) * 5, 2)
...
def analyse_innovation_growth(ticker: str) -> Dict[str, Any]:
    """
    ...
    r_and_d_values = filter_valid_values(research_and_development) if research_and_development is not None else []
    revenue_values = filter_valid_values(revenue) if revenue is not None else []

    if r_and_d_values and revenue_values and len(r_and_d_values) >= 2:
        rd_growth = safe_divide(
            r_and_d_values[0] - r_and_d_values[-1],
            abs(r_and_d_values[-1])
        )
        ...
        if len(r_and_d_values) > 0 and len(revenue_values) > 0:
            rd_intensity_start = safe_divide(r_and_d_values[-1], revenue_values[-1])
            rd_intensity_end = safe_divide(r_and_d_values[0], revenue_values[0])
    ...
    fcf_values = filter_valid_values(free_cash_flow) if free_cash_flow is not None else []

    if fcf_values and len(fcf_values) >= 2:
        fcf_growth = safe_divide(
            fcf_values[0] - fcf_values[-1],
            abs(fcf_values[-1])
        )
    ...
    if operating_income is not None and not operating_income.empty and total_revenue is not None and not total_revenue.empty:
        operating_income_values = filter_valid_values(operating_income)
        total_revenue_values = filter_valid_values(total_revenue)

        if len(operating_income_values) == len(total_revenue_values) and len(operating_income_values) > 0:
            operating_margins = [
                safe_divide(operating_income_values[i], total_revenue_values[i])
                for i in range(len(operating_income_values))
            ]

            op_margins = [m for m in operating_margins if m is not None and not np.isnan(m)]

            if op_margins and len(op_margins) >= 2:
                margin_trend = op_margins[0] - op_margins[-1]

                if op_margins[0] > OPERATING_MARGIN_STRONG and margin_trend > 0:
                    score += 3
                    details.append(
                        f"Strong and improving operating margin: {(op_margins[0] * 100):.1f}%"
                    )
                elif op_margins[0] > OPERATING_MARGIN_ACCEPTABLE:
                    score += 2
                    details.append(
    ...
    capex = filter_valid_values(capital_expenditure) if capital_expenditure is not None else []

    if capex and len(capex) >= 2 and revenue_values and len(revenue_values) > 0:
        capex_intensity = safe_divide(abs(capex[0]), revenue_values[0])
        capex_growth = safe_divide(
            abs(capex[0]) - abs(capex[-1]),
            abs(capex[-1])
        )

        if capex_intensity > CAPEX_INTENSITY_MODERATE and capex_growth > 0.2:
            score += 2
    ...
    dividends = filter_valid_values(cash_dividends) if cash_dividends is not None else []

    if dividends and fcf_values and len(fcf_values) > 0:
        latest_payout_ratio = safe_divide(dividends[0], fcf_values[0], 1)
...
def analyse_cathie_wood_valuation(
    ...
    Uses a simplified DCF approach with high growth assumptions.
    ...
    if market_cap is None or np.isnan(market_cap) or market_cap <= 0:
        return create_error_result("Market cap data not available or invalid")
    ...
    if not fcf_values:
        return {
            ...
            "details": "No FCF data available for valuation",
            "intrinsic_value": "Not possible to accurately estimate with this methodology",
        }

    fcf = fcf_values[0] * exchange_rate
    ...
    # DCF with high growth assumptions for innovative companies
    present_value = 0
    ...
    margin_of_safety = safe_divide(intrinsic_value - market_cap, market_cap)

    if margin_of_safety > MARGIN_OF_SAFETY_GOOD:
        score += 3
    elif margin_of_safety > MARGIN_OF_SAFETY_ACCEPTABLE:
        score += 1
...
if __name__ == "__main__":
    analysis = calculate_cathie_wood_analysis_data("MSFT")
    pprint(analysis)
