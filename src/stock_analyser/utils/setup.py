...
class Setup:
    ...
    def initialize_analysis(self):
        ...
        """
        print(
            f"Initializing analysis for {self.state.company_name} ({self.state.company_ticker})..."
        )
        ...
        if self.qdrant_client:
            if same_company:
                ...
                missing_filing_types = [
                    ft for ft, needed in missing_filings.items() if needed
                ]

                if missing_filing_types:
                    print(
                        f"Downloading missing filings: {', '.join(missing_filing_types)}"
                    )
                ...
                print("New company analysis. Downloading all filings.")
    ...
    def check_knowledge_dir(self):
        ...
        return (
            marker_exists and not archive_needed
        )  # Return True if same ticker (no archive needed)
    ...
    def validate_qdrant_collection(self):
        ...
        if not self.qdrant_client:
            ...
            return {"10-Q": True, "10-K": True, "8-K": True, "6-K": True}

        filing_types = ["10-Q", "10-K", "8-K", "6-K"]
        missing_filings = {}

        collection_exists = self.qdrant_client.collection_exists(self.collection_name)
        print(
            f"Checking if collection '{self.collection_name}' exists: {collection_exists}"
        )
    ...
    def download_filings(self, filing_types_to_download=None):
        ...
        """
        all_filing_types = ["10-Q", "10-K", "8-K", "6-K"]
        ...
        print(
            f"Downloading SEC filings for {self.state.company_name} ({self.state.company_ticker}): {', '.join(filing_types)}"
        )
        ...
        for filing_type in filing_types:
            try:
                file_path = (
                    FILINGS_DIR / f"{self.state.company_ticker}_{filing_type}.txt"
                )
                try:
                    filings = get_most_recent_filing(
                        self.state.company_ticker, filing_type
                    )
                except Exception as e:
                ...
                if not filings:  # Check for empty content *before* saving
                    print(
                        f"No {filing_type} filings found or empty for {self.state.company_ticker}"
                    )
                    success = False
                    continue
                ...
                knowledge_file_path = (
                    KNOWLEDGE_DIR / f"{self.state.company_ticker}_{filing_type}.md"
                )
                try:
